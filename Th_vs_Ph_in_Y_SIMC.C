...
void Th_vs_Ph_in_Y_SIMC() 
{
  ...
  T->Add("/home/skbarcus/Tritium/Simulations/SIMC/simc_gfortran.12/output/P0_3.055_6GeV/Ly50_Lx70_Ld6_wide_2body_xp_-25_yp_-1_10M.root");
  ...
  */
  ...
  Int_t nslices = 10;
  Double_t yplot_min=-5.,yplot_max=5.;
  ...
  T->Draw(Form("e_ytar>>h1(%d,%f,%f)",nbins,yplot_min,yplot_max),ct_norm);//,ct_1tr&&ct_trg&&ct_th&&ct_ph&&ct_y&&ct_pr&&ct_gc&&ct_dp);
  h1->SetTitle("e_ytar");
  h1->GetXaxis()->SetTitle("e_ytar");

  //Draw theta vs phi with no cuts.
  c1->cd(2);
  T->Draw(Form("e_yptar:e_xptar>>h2(%d,%f,%f,%d,%f,%f)",nbins,thplot_min,thplot_max,nbins,phplot_min,phplot_max));
  ...
  for(Int_t i=0;i<nslices;i++)
    {
      c1->cd(3+i);
      T->Draw(Form("e_yptar:e_xptar>>h%d(%d,%f,%f,%d,%f,%f)",3+i,nbins,thplot_min,thplot_max,nbins,phplot_min,phplot_max),Form("e_ytar>%f&&e_ytar<%f",yplot_min+i*ystep,yplot_min+(i+1)*ystep));
    }
